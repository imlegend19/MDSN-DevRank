...
with db:
    ...
    cur.execute("select assigned_to, bug_severity, count(*) from test_bug_fixed_closed "
    ...
    for i in cur.fetchall():
        if i[0].strip() in segregated_assignees:
            if segregated_assignees[i[0].strip()] in assignee_severity_cnt:
                dic = assignee_severity_cnt[segregated_assignees[i[0].strip()]]
                dic[i[1]] = i[2]
                assignee_severity_cnt[segregated_assignees[i[0].strip()]] = dic
            else:
                assignee_severity_cnt[segregated_assignees[i[0].strip()]] = {i[1]: i[2]}
