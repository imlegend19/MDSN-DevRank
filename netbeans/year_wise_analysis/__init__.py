...
def calculate_avg_fixed(year):
    with db:
        ...
        cur = db.cursor()
        cur.execute(
            "SELECT DISTINCTROW bug_id, assigned_to FROM test_bugs_fixed_closed where year(creation_ts)={0}".format(
                year))
...
def calculate_priority(year):
    with db:
        cur = db.cursor()
        cur.execute(
            "SELECT DISTINCT assigned_to FROM test_bugs_fixed_closed WHERE assigned_to IN (SELECT who FROM test_longdescs_fixed_closed) and year(creation_ts)=" + str(
                year))
        ...
        cur.execute(
            "select assigned_to, priority, count(*) from test_bugs_fixed_closed where year(creation_ts)=%d group by assigned_to, priority" % (
                year))
...
def calculate_severity(year):
    with db:
        ...
        cur.execute(
            "select assigned_to, bug_severity, count(*) from test_bugs_fixed_closed where year(creation_ts)=%d group by assigned_to, bug_severity" % (
                year))
...
def calculate_reopened(year):
    with db:
        ...
        cur.execute(
            "SELECT DISTINCTROW bug_id, assigned_to FROM test_bugs_fixed_closed where year(creation_ts)={}".format(
                year))
...
if __name__ == '__main__':
    ...
    titles = ['Year', 'Assignee', 'L1', 'L2 - D1', 'L2 - D2', 'L3', 'L4', 'Avg Fixed', 'Avg Closed', 'Avg Reopened',
              'Priority Points', 'Severity Points']
    ...
    for i in years:
        ...
        l2_d1_centrality = layer_2_d1(2001, i)
        l2_d2_centrality = layer_2_d2(2001, i)
        l3_centrality = layer_3(2001, i)
        l4_centrality = layer_4(2001, i)
        avg_fixed = calculate_avg_fixed(i + 1)
        avg_closed = calculate_avg_closed(i + 1)
        avg_reopened = calculate_reopened(i + 1)
        priority = calculate_priority(i + 1)
        severity = calculate_severity(i + 1)
